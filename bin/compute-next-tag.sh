...
#
# Tags look like `v<Docker Registry Proxy version>-<release>`:
#
# - if defaults/main.yml points at a Docker Registry Proxy version that has
#   never been released, the release counter restarts at 0 (`v1.2.8-0`)
# - otherwise the counter is incremented (`v1.2.8-1`), but only if something
...
# above the variable, nor a commented-out version, nor any of the several
# variables derived from this one is mistaken for the version itself.
version="$(sed -nE 's|^docker_registry_proxy_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Docker Registry Proxy version from $defaults_path"
	exit 1
fi

# The version values already carry a leading `v` (e.g. `v1.2.7`),
